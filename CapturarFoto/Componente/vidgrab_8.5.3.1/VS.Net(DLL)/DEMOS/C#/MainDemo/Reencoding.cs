...
namespace MainDemo
{
	...
	public class Reencoding : SubForm // SubForm is declared in CSUtil. Unlike default forms, it is not destroyed when closed, and includes also some additional tools.
    {
		...
		{
			SetRadioButtonInGroupUsingTag((rdgReencodingMethod), (short) cMainForm.VideoGrabber1.Reencoding_Method);
			chkWMVOutput.Checked = cMainForm.VideoGrabber1.Reencoding_WMVOutput;
			chkIncludeVideoStream.Checked = cMainForm.VideoGrabber1.Reencoding_IncludeVideoStream;
			chkIncludeAudioStream.Checked = cMainForm.VideoGrabber1.Reencoding_IncludeAudioStream;
			chkUseFrameGrabber.Checked = cMainForm.VideoGrabber1.Reencoding_UseFrameGrabber;
			chkUseCurrentVideoCompressor.Checked = cMainForm.VideoGrabber1.Reencoding_UseVideoCompressor;
			chkUseCurrentAudioCompressor.Checked = cMainForm.VideoGrabber1.Reencoding_UseAudioCompressor;
			edtStartTime.Text = cMainForm.VideoGrabber1.Reencoding_StartTime.ToString();
			edtStopTime.Text = cMainForm.VideoGrabber1.Reencoding_StopTime.ToString();
		}
		...
		private void btnDuration_Click(object sender, System.EventArgs e)
		{
			...
			if (cMainForm.VideoGrabber1.AVIDuration(edtSourceVideoClip.Text,out AVIDuration,out AVIFrameCount)) 
			{
		...
		{
			cMainForm.VideoGrabber1.Reencoding_SourceVideoClip = edtSourceVideoClip.Text;
			cMainForm.VideoGrabber1.Reencoding_NewVideoClip = edtDestinationVideoClip.Text;

			cMainForm.VideoGrabber1.Reencoding_WMVOutput = chkWMVOutput.Checked;
			cMainForm.VideoGrabber1.Reencoding_IncludeVideoStream = chkIncludeVideoStream.Checked;
			cMainForm.VideoGrabber1.Reencoding_IncludeAudioStream = chkIncludeAudioStream.Checked;
			cMainForm.VideoGrabber1.Reencoding_UseFrameGrabber = chkUseFrameGrabber.Checked;
			cMainForm.VideoGrabber1.Reencoding_UseVideoCompressor = chkUseCurrentVideoCompressor.Checked;
			cMainForm.VideoGrabber1.Reencoding_UseAudioCompressor = chkUseCurrentAudioCompressor.Checked;

			if (rdgReencodingMethod1.Checked) 
			{
				cMainForm.VideoGrabber1.Reencoding_Method = VidGrab.TRecordingMethod.rm_AVI;
			}

			if (rdgReencodingMethod2.Checked) 
			{
				;
				cMainForm.VideoGrabber1.Reencoding_Method = VidGrab.TRecordingMethod.rm_ASF;
			};

			if (IsNumeric(edtStartTime.Text)) 
			{
				;
				cMainForm.VideoGrabber1.Reencoding_StartTime = Convert.ToInt64 (edtStartTime.Text);
			};

			if (IsNumeric(edtStopTime.Text)) 
			{
				;
				cMainForm.VideoGrabber1.Reencoding_StopTime = Convert.ToInt64 (edtStopTime.Text);
			};

			if (IsNumeric(edtStartFrame.Text)) 
			{
				;
				cMainForm.VideoGrabber1.Reencoding_StartFrame = Convert.ToInt64 (edtStartFrame.Text);
			};

			if (IsNumeric(edtStopFrame.Text)) 
			{
				;
				cMainForm.VideoGrabber1.Reencoding_StopFrame = Convert.ToInt64 (edtStopFrame.Text);
			};

			cMainForm.VideoGrabber1.StartReencoding();
		}

		private void btnStopReencoding_Click(object sender, System.EventArgs e)
		{
			cMainForm.VideoGrabber1.StopReencoding();
		}
