...
namespace MainDemo
{
	...
	public class Audio : SubForm // SubForm is declared in CSUtil. Unlike default forms, it is not destroyed when closed, and includes also some additional tools.
    {
        ...
        {
            AssignListToComboBox(cboAudioDevices, cMainForm.VideoGrabber1.AudioDevices, cMainForm.VideoGrabber1.AudioDevice);

            if (cMainForm.VideoGrabber1.AudioDevice >= 0) { // if an audio capture device has been selected
                AssignListToComboBox(cboAudioInputs, cMainForm.VideoGrabber1.AudioInputs, cMainForm.VideoGrabber1.AudioInput);
            }

            tbrAudioInputLevel.Value = cMainForm.VideoGrabber1.AudioInputLevel;
            tbrAudioInputBalance.Value = cMainForm.VideoGrabber1.AudioInputBalance;
            chkAudioInputMono.Checked = cMainForm.VideoGrabber1.AudioInputMono;
		
			AssignListToComboBox(cboAudioRenderers, cMainForm.VideoGrabber1.AudioRenderers, cMainForm.VideoGrabber1.AudioRenderer);

			chkRenderAudioDevice.Checked = cMainForm.VideoGrabber1.AudioDeviceRendering;
			chkMuteAudioRendering.Checked = cMainForm.VideoGrabber1.MuteAudioRendering;

			if (cMainForm.VideoGrabber1.AudioDevice == -1)
			{
				cboAudioDevices.Text = "SELECT AND AUDIO CAPTURE DEVICE IN THIS LIST";
			}
			if (cMainForm.VideoGrabber1.AudioRenderer == -1) {
		...
		private void rdgVUMeter_CheckedChanged(object sender, System.EventArgs e)
		{
			if ((sender as RadioButton).Checked) 
			{
				...
				{
					case 0:
						cMainForm.VideoGrabber1.VuMeter = VidGrab.TVuMeter.vu_Disabled;
						cMainForm.VideoGrabber1.SetVUMeterSetting(Convert.ToUInt32(VU_LEFT), VidGrab.TVUMeterSetting.vu_Handle, Convert.ToUInt32(0)); // VU_LEFT = 0, VU_RIGHT = 1;
						cMainForm.VideoGrabber1.SetVUMeterSetting(Convert.ToUInt32(VU_RIGHT), VidGrab.TVUMeterSetting.vu_Handle, Convert.ToUInt32(0));
					break;
					case 1:
						cMainForm.VideoGrabber1.VuMeter = VidGrab.TVuMeter.vu_Analog;
			
						pnlVUMeterLeft.Width = (pnlVUMeterLeft.Height * 4) / 3;
						pnlVUMeterRight.Width = (pnlVUMeterRight.Height * 4) / 3;
			
						cMainForm.VideoGrabber1.SetVUMeterSetting(Convert.ToUInt32(VU_LEFT), VidGrab.TVUMeterSetting.vu_BkgndColor, Convert.ToUInt32(0)); // VU_LEFT = 0, VU_RIGHT = 1
						cMainForm.VideoGrabber1.SetVUMeterSetting(Convert.ToUInt32(VU_RIGHT), VidGrab.TVUMeterSetting.vu_BkgndColor, Convert.ToUInt32(0));
						cMainForm.VideoGrabber1.SetVUMeterSetting(Convert.ToUInt32(VU_LEFT), VidGrab.TVUMeterSetting.vu_Handle, Convert.ToUInt32(pnlVUMeterLeft.Handle.ToInt32())); // VU_LEFT = 0, VU_RIGHT = 1
						cMainForm.VideoGrabber1.SetVUMeterSetting(Convert.ToUInt32(VU_RIGHT), VidGrab.TVUMeterSetting.vu_Handle, Convert.ToUInt32(pnlVUMeterRight.Handle.ToInt32()));
						break;
					case 2:
						cMainForm.VideoGrabber1.VuMeter = VidGrab.TVuMeter.vu_Bargraph;
			
						pnlVUMeterLeft.Width = (pnlVUMeterLeft.Height * 2) / 10;
						pnlVUMeterRight.Width = (pnlVUMeterRight.Height * 2) / 10;
			
						cMainForm.VideoGrabber1.SetVUMeterSetting(Convert.ToUInt32(VU_LEFT), VidGrab.TVUMeterSetting.vu_BkgndColor, Convert.ToUInt32(0x404040)); // VU_LEFT = 0, VU_RIGHT = 1
						cMainForm.VideoGrabber1.SetVUMeterSetting(Convert.ToUInt32(VU_RIGHT), VidGrab.TVUMeterSetting.vu_BkgndColor, Convert.ToUInt32(0x404040));
						cMainForm.VideoGrabber1.SetVUMeterSetting(Convert.ToUInt32(VU_LEFT), VidGrab.TVUMeterSetting.vu_Handle, Convert.ToUInt32(pnlVUMeterLeft.Handle.ToInt32())); // VU_LEFT = 0, VU_RIGHT = 1
						cMainForm.VideoGrabber1.SetVUMeterSetting(Convert.ToUInt32(VU_RIGHT), VidGrab.TVUMeterSetting.vu_Handle, Convert.ToUInt32(pnlVUMeterRight.Handle.ToInt32()));
						break;
				}
		...
		{
			cMainForm.VideoGrabber1.StartAudioRendering();
			if (! cMainForm.VideoGrabber1.AudioDeviceRendering) {
		...
		{
			cMainForm.VideoGrabber1.CompressionMode = VidGrab.TCompressionMode.cm_NoCompression;
			cMainForm.VideoGrabber1.RecordingMethod = VidGrab.TRecordingMethod.rm_AVI;
			cMainForm.VideoGrabber1.StartAudioRecording();
		}

		private void btnSartMP3Recording_Click(object sender, System.EventArgs e)
		{
			cMainForm.VideoGrabber1.CompressionMode = VidGrab.TCompressionMode.cm_CompressOnTheFly;
			cMainForm.VideoGrabber1.CompressionType = VidGrab.TCompressionType.ct_Audio;
			cMainForm.VideoGrabber1.RecordingMethod = VidGrab.TRecordingMethod.rm_AVI;
			
			cMainForm.VideoGrabber1.AudioCompressor = cMainForm.VideoGrabber1.FindIndexInListByName (cMainForm.VideoGrabber1.AudioCompressors, "LAME Audio Encoder", false, true);
			if (cMainForm.VideoGrabber1.AudioCompressor == -1) 
			{
		...
		{
			cMainForm.VideoGrabber1.CompressionMode = VidGrab.TCompressionMode.cm_NoCompression;
			cMainForm.VideoGrabber1.RecordingMethod = VidGrab.TRecordingMethod.rm_ASF;
			cMainForm.VideoGrabber1.StartAudioRecording();
		}
		...
		private void chkUseAudioOfVideoCaptureDevice_CheckedChanged(object sender, System.EventArgs e)
		{
			...
			{
				cMainForm.VideoGrabber1.VideoDevice = 0; // in this demo, by default we choose the 1st
			}
			else {
				cMainForm.VideoGrabber1.VideoDevice = -1; // this deactivates the video capture device
			}
		}
