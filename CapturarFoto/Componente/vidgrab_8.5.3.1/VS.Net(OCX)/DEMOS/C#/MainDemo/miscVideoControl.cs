...
namespace MainDemo
{
	...
	public class miscVideoControl : SubForm // SubForm is declared in CSUtil. Unlike default forms, it is not destroyed when closed, and includes also some additional tools.
	{
		...
		{
			chkFlipVert.Enabled = cMainForm.axVideoGrabberNET1.IsVideoControlModeAvailable(Vidgrab_NET.TxVideoControl.vc_FlipHorizontal);
			chkFlipHorz.Enabled = cMainForm.axVideoGrabberNET1.IsVideoControlModeAvailable(Vidgrab_NET.TxVideoControl.vc_FlipVertical);
			chkExternalTrigger.Enabled = cMainForm.axVideoGrabberNET1.IsVideoControlModeAvailable(Vidgrab_NET.TxVideoControl.vc_ExternalTriggerEnable);
			chkEmulateTrigger.Enabled = cMainForm.axVideoGrabberNET1.IsVideoControlModeAvailable(Vidgrab_NET.TxVideoControl.vc_Trigger);
		}

		private void chk_CheckedChanged(object sender, System.EventArgs e)
		{
			cMainForm.axVideoGrabberNET1.SetVideoControlMode(chkFlipHorz.Checked, chkFlipVert.Checked, chkExternalTrigger.Checked, chkEmulateTrigger.Checked);
		}
